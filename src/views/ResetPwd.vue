<template>
  <div class="eva-single-page-wrapper text-center">
    <form v-if="step === 1">
      ...
      <p class="text-warning" v-if="request.state === 'success'">一封包含重置链接的邮件已经发送到您填写的邮箱，请查看邮件并跟随邮件内指引操作。</p>
      <p class="text-warning" v-else-if="request.state === 'Request failed with status code 404'">没有该邮箱的注册记录</p>
      <p class="text-warning" v-else-if="request.state !== ''">发生未知错误，请刷新后重试</p>
    ...
    <form v-else>
      ...
      <p class="text-warning" v-else-if="reset.state === 'success'">密码重置成功，请<router-link :to="{name: 'Signin'}">重新登陆</router-link></p>
      <p class="text-warning" v-else-if="reset.state === 'password not valid'">新密码格式错误，至少需要8位，包含小写字母及数字</p>
      <p class="text-warning" v-else-if="reset.state !== ''">发生未知错误，请刷新后重试</p>
